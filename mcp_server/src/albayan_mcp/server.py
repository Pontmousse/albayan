...
def create_server() -> MCPServer:
    ...
    server = MCPServer(
        ...
        instructions=(
            ...
            "معرّفات الكتل أو الحقول أو الرموز أو القوائم أو الجداول. للمعادلات الموجودة استخدم "
            "get_draft_equations؛ فهو يعرض LaTeX إنجليزيًا قانونيًا للتوافق مع الذكاء "
            "الاصطناعي داخل مقال رياضي عربي، مع اصطلاحات الرموز العربية الحالية. قبل إنشاء "
            "أي معادلة جديدة استدع get_math_authoring_capabilities، والتزم فقط بقسم "
            "round_trip_safe من LaTeX القانوني العادي، ولا تُنشئ ماكروات Burhan/BuTeX "
            "العربية الداخلية. استخدم أحدث revision كـ base_revision، وعند revision_conflict "
            "أعد قراءة المسودة قبل المحاولة. خصّص command_id واحدًا لكل تعديل منطقي ولا تعِد "
            "استخدامه لحمولة مختلفة. فضّل أمر Document2 موجهاً على إنشاء مستند خام كامل. "
            "لا تستدع compile_draft إلا بطلب صريح؛ فهو ينشئ المعاينة من المراجعة الموثوقة "
            "على الخادم، دون إنشاء LaTeX أو مفاتيح أصول أو hash. افحص get_compile_status "
            "قبل طلب PDF. للصور، استخدم list_article_assets قبل الاسترجاع، وارفع فقط ملفاً "
            "اختاره المستخدم. بعد الرفع مرّر asset_id العائد إلى insert_figure أو "
            "update_figure، ولا تخترع مسار أصل. يمكن إنشاء المسودات وتحديث عنوانها وملخصها "
            "عبر أدوات المقال المخصصة؛ لا تعدّل authors داخل Document2، ولا توجد أداة تقديم "
            "أو حذف أو إدارة مؤلفين. يستطيع المؤلف البشري مراجعة سجل المسودة واستعادة لقطة "
            "سابقة من الواجهة؛ لا توجد أداة استعادة للوكيل، وتبقى تعديلاته ظاهرة في السجل "
            "مع مصدرها. أدوات المسودة متاحة أيضاً أثناء حالة revision_requested كي يساعد "
            "الوكيل في التعديل، لكن طلب التعديلات وإعادة التقديم والقرارات وكشف هويات "
            "المراجعين تبقى عمليات بشرية فقط."
        ),
        auth=auth,
