"""Independent MCP mirror of the FastAPI Document2 command contract."""
...
class Document2ContractModel(BaseModel):
    """Strict command model matching the FastAPI/BuTeX wire contract."""
    ...
    def _reject_explicit_null_for_optional_wire_fields(self):
        for field_name in self.model_fields_set:
            if getattr(self, field_name) is None and field_name not in self.nullable_fields:
                raise ValueError(f"{field_name} لا يقبل null عند إرساله.")
        return self
...
class DocumentAnchor(Document2ContractModel):
    ...
    def _exactly_one_anchor(self):
        values = (self.before_block_id, self.after_block_id, self.end)
        if sum(value is not None for value in values) != 1:
            raise ValueError("حدد before_block_id أو after_block_id أو end: true فقط.")
        return self
...
class DocumentInlineAnchor(Document2ContractModel):
    ...
    @model_validator(mode="after")
    def _exactly_one_anchor(self):
        values = (self.before_token_id, self.after_token_id, self.start, self.end)
        if sum(value is not None for value in values) != 1:
...
class DocumentListItemAnchor(Document2ContractModel):
    ...
    @model_validator(mode="after")
    def _exactly_one_anchor(self):
        values = (self.before_item_id, self.after_item_id, self.end)
        if sum(value is not None for value in values) != 1:
...
class DocumentReferenceAnchor(Document2ContractModel):
    ...
    @model_validator(mode="after")
    def _exactly_one_anchor(self):
        values = (self.before_reference_key, self.after_reference_key, self.end)
        if sum(value is not None for value in values) != 1:
...
class DocumentMathNodeJson(Document2ContractModel):
    nullable_fields: ClassVar[frozenset[str]] = frozenset({"superscript", "subscript"})
...
def _require_non_empty_key(keys: list[str]) -> list[str]:
    if not any(key.strip() for key in keys):
        raise ValueError("يجب تحديد مفتاح واحد غير فارغ على الأقل.")
    return keys
...
class UpdateFloatMetaCommand(Document2ContractModel):
    ...
    @model_validator(mode="after")
    def _at_least_one_patch_field(self):
        fields = {"centered", "caption_enabled", "caption", "label_enabled", "label"}
        if not self.model_fields_set.intersection(fields):
            raise ValueError("update_float_meta يتطلب حقل تعديل واحدًا.")
        return self
...
class DocumentHijriDatePatch(Document2ContractModel):
    ...
    def _at_least_one_date_field(self):
        if not self.model_fields_set.intersection({"day", "month", "year"}):
            raise ValueError("تحديث التاريخ يتطلب حقلًا واحدًا على الأقل.")
        return self
...
class UpdateDocumentMetaCommand(Document2ContractModel):
    ...
    @model_validator(mode="after")
    def _at_least_one_patch_field(self):
        if not self.model_fields_set.intersection({"title", "authors", "abstract", "date"}):
            raise ValueError("update_document_meta يتطلب حقل تعديل واحدًا.")
        return self
...
class UpdateReferenceCommand(Document2ContractModel):
    ...
    @model_validator(mode="after")
    def _at_least_one_patch_field(self):
        fields = {"key", "authors", "title", "year", "venue", "url", "field_separator"}
        if not self.model_fields_set.intersection(fields):
            raise ValueError("update_reference يتطلب حقل تعديل واحدًا.")
        return self
...
class InsertTableCommand(Document2ContractModel):
    ...
    def _rectangular_non_empty_rows(self):
        ...
        if width == 0 or any(len(row) != width for row in self.rows):
            raise ValueError("يجب أن تكون صفوف الجدول مستطيلة وغير فارغة.")
        return self
