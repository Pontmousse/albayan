...
Account-security messages generated by Clerk have an explicit delivery-owner split. The production ownership matrix, approved Arabic copy, branding rules, and Dashboard procedure live in [`clerk-security-email-templates.md`](./clerk-security-email-templates.md).

The key invariant is **one delivery owner per event**:

- ordinary account-security notifications stay Clerk-delivered;
- email-verification and password-reset OTPs are the only families currently intercepted by Albayan and sent through Resend;
- Clerk delivery for the password-reset-code template must remain disabled while the `password-reset-ar` Resend path is active.

Do not add a new Clerk-email webhook handler without reviewing the corresponding Clerk delivery toggle so the same security event cannot be sent twice.
