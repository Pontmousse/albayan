    ...
    setRows((current) =>
      (current ?? []).map((row, rowIndex) => {
        ...
        return {
          ...row,
          arabic: value,
          fontId: row.fontId === "custom" ? "default" : row.fontId,
  ...
  return (
    ...
    >
      <header className="flex shrink-0 items-center justify-between gap-3 border-b border-[var(--journal-border)] bg-white/85 px-4 py-3 sm:px-5 md:px-6 md:py-5">
        <div className="min-w-0">
          ...
          <p className="mt-1 hidden text-sm leading-6 text-slate-600 sm:block">
            اختر المقابل العربي لكل رمز، وأضف خطاً خاصاً فقط عندما تريده.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain lg:grid lg:grid-cols-[18rem_minmax(0,1fr)] lg:overflow-hidden">
        <div className="mx-4 mt-4 rounded-xl border border-emerald-200/80 bg-emerald-50/80 px-4 py-3 lg:hidden">
          <div className="flex items-start gap-2.5">
            ...
            <div className="min-w-0">
              <p className="text-sm leading-6 text-emerald-950">
                اختر الصيغة العربية التي تفضّلها لرموز مقالك. تساعد هذه الاختيارات أدوات البيان الذكية على فهم اصطلاحاتك الرياضية بصورة أدق، وتُسهم في تطوير دعم أفضل للرياضيات المكتوبة بالعربية.
              </p>
        ...
        <aside className="hidden border-l border-[var(--journal-border)] bg-gradient-to-b from-[var(--journal-accent-soft)]/75 to-white px-6 py-6 lg:block">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-6 text-slate-600">
                عيّن لكل رمز أصلي قيمته العربية. اترك الخط على «الافتراضي» ما لم ترغب في خط خاص لهذا الرمز.
              </p>
            </div>
          </div>
          ...
          <div className="mt-4 rounded-xl border border-emerald-200/80 bg-emerald-50/80 p-3.5 text-sm leading-6 text-emerald-950">
            <div className="flex items-start gap-2.5">
              ...
              />
              <p>
                تساعد اختياراتك أدوات البيان الذكية على فهم اصطلاحات المقال بصورة أدق، وتُسهم في تطوير أدوات ونماذج مستقبلية أكثر فهماً للرياضيات العربية.
              </p>
            </div>
          </div>
            ...
            {rows === null ? (
              ...
              <div className="flex min-h-64 flex-col items-center justify-center rounded-2xl border border-dashed border-[var(--journal-border)] bg-white/80 p-6 text-center">
                ...
                <p className="mt-1 text-sm text-slate-500">
                  مثال: x ← س، واترك الخط على «الافتراضي» إذا لم ترد خطاً خاصاً.
                </p>
                <button
                  type="button"
                  onClick={addRow}
                      ...
                      <div
                        className={`mt-3 grid gap-3 ${
                          showTechnicalPreview
                            ? "md:grid-cols-[minmax(0,15rem)_minmax(0,1fr)] md:items-start"
                            : "md:max-w-[20rem]"
                        }`}
                      >
                        ...
                        {showTechnicalPreview ? (
                          <details className="rounded-lg border border-amber-200 bg-amber-50/70 px-3 py-2 text-xs text-amber-900">
                            <summary className="cursor-pointer font-semibold">
                              معاينة تقنية — وضع التطوير
                            </summary>
